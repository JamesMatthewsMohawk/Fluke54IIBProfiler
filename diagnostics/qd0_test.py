...
LOG_PATH = Path("logs") / "qd0_test.log"
...
def main() -> int:
    ...
    lines = [f"=== qd0_test start {datetime.now().isoformat(timespec='seconds')} ==="]
    ...
    with ser:
        ...
        line = (f"sent HEX {hex_dump(payload)} -- got {len(response)} bytes: "
                f"HEX {hex_dump(response)} ASCII {ascii_dump(response)!r}")
        print(line)
        lines.append(line)

    lines.append("=== qd0_test end ===")
    LOG_PATH.write_text("\n".join(lines), encoding="utf-8")
    return 0
