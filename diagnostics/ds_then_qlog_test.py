"""Test: send DS\\r, wait 200ms, then send QLOG\\r, and see what comes back.

DS (Default Setup) is documented in sibling Fluke families as resetting
certain settings or being "equivalent to cycling instrument power" -- not
memory-clearing like RI. Never sent to this specific meter before. This
checks whether it changes what commands the meter accepts afterward.
"""
...
POST_DS_DELAY_S = 0.2
LOG_PATH = Path("logs") / "ds_then_qlog_test.log"
...
def main() -> int:
    ...
    lines = [f"=== ds_then_qlog_test start {datetime.now().isoformat(timespec='seconds')} ==="]
    ...
    with ser:
        # Baseline check before DS, to know starting state
        send_and_capture(ser, "ID", lines)
        time.sleep(0.3)

        send_and_capture(ser, "DS", lines)
        time.sleep(POST_DS_DELAY_S)
        send_and_capture(ser, "QLOG", lines)
        time.sleep(0.3)

        # Follow-up baseline check to see if the meter is still responsive at all
        send_and_capture(ser, "ID", lines)

    lines.append("=== ds_then_qlog_test end ===")
    LOG_PATH.write_text("\n".join(lines), encoding="utf-8")
    return 0
