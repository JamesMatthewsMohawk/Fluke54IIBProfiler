"""Test QD <index> for a range of indices, one at a time, logging full results."""
...
LOG_PATH = Path("logs") / "qd_index_test.log"
...
def main() -> int:
    ...
    lines = [f"=== qd_index_test start {datetime.now().isoformat(timespec='seconds')} ==="]
    ...
    with ser:
        for idx in INDICES:
            ...
            line = (f"QD {idx}: sent HEX {hex_dump(payload)} -- got {len(response)} bytes: "
                    f"HEX {hex_dump(response)} ASCII {ascii_dump(response)!r}")
            print(line)
            lines.append(line)
            time.sleep(INTER_COMMAND_DELAY_S)

    lines.append("=== qd_index_test end ===")
    LOG_PATH.write_text("\n".join(lines), encoding="utf-8")
    return 0
