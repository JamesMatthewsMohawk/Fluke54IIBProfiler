"""Direct confirmation test for QLOG and NLOG, requested explicitly by the user."""
...
LOG_PATH = Path("logs") / "qlog_nlog_test.log"

CANDIDATES = ["QLOG", "NLOG"]
...
def main() -> int:
    ...
    lines = [f"=== qlog_nlog_test start {datetime.now().isoformat(timespec='seconds')} ==="]
    ...
    lines.append("=== qlog_nlog_test end ===")
    LOG_PATH.write_text("\n".join(lines), encoding="utf-8")
    return 0
