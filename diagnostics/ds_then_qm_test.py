"""Test: send DS\\r, wait 200ms, then send QM\\r, and see what comes back."""
...
POST_DS_DELAY_S = 0.2
LOG_PATH = Path("logs") / "ds_then_qm_test.log"
...
def main() -> int:
    ...
    lines = [f"=== ds_then_qm_test start {datetime.now().isoformat(timespec='seconds')} ==="]
    ...
    lines.append("=== ds_then_qm_test end ===")
    LOG_PATH.write_text("\n".join(lines), encoding="utf-8")
    return 0
