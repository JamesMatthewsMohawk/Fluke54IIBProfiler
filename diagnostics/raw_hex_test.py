"""Send an exact raw hex byte sequence, no string encoding involved at all."""
...
LOG_PATH = Path("logs") / "raw_hex_test.log"

RAW_PAYLOAD = bytes([0x51, 0x44, 0x44, 0x41, 0x0D])
...
def main() -> int:
    ...
    lines = [f"=== raw_hex_test start {datetime.now().isoformat(timespec='seconds')} ===",
             f"Raw payload: {hex_dump(RAW_PAYLOAD)} (ASCII interpretation: {ascii_dump(RAW_PAYLOAD)!r})"]
