"""Clean, dedicated test of the 'does ID wake up QM/DL/DUMP' hypothesis.

Single continuous serial connection, in order:
    ID -> QM -> DL -> DUMP

All four have been sent before in earlier sessions (individually and in
sequence), but this is a minimal, purpose-built run isolated from noisier
sweep logs, to give an unambiguous answer.
"""
...
LOG_PATH = Path("logs") / "wakeup_hypothesis_test.log"

SEQUENCE = ["ID", "QM", "DL", "DUMP"]
...
def main() -> int:
    ...
    lines = [f"=== wakeup_hypothesis_test start {datetime.now().isoformat(timespec='seconds')} ===",
              f"Sequence (single continuous connection): {SEQUENCE}"]
    ...
    lines.append("=== wakeup_hypothesis_test end ===")
    LOG_PATH.write_text("\n".join(lines), encoding="utf-8")
    return 0
