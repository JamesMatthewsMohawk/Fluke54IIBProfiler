"""Test: send DS\\r, wait 200ms, then send QDDA\\r, and see what comes back."""
...
POST_DS_DELAY_S = 0.2
LOG_PATH = Path("logs") / "ds_then_qdda_test.log"
...
def main() -> int:
    ...
    lines = [f"=== ds_then_qdda_test start {datetime.now().isoformat(timespec='seconds')} ==="]
    ...
    with ser:
        ...
        send_and_capture(ser, "QDDA", lines)
        time.sleep(0.3)

        send_and_capture(ser, "ID", lines)

    lines.append("=== ds_then_qdda_test end ===")
    LOG_PATH.write_text("\n".join(lines), encoding="utf-8")
    return 0
