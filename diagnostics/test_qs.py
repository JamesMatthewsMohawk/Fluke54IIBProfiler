"""Single, careful retest of the 'QS' command with a long capture window.

Sends QS\\r exactly once and captures up to 10 seconds of response, so we can
see the full (previously truncated) payload. Does not send any other command.
"""
...
LOG_PATH = Path("logs") / "test_qs.log"
...
def main() -> int:
    ...
    lines = [f"=== test_qs session start {datetime.now().isoformat(timespec='seconds')} ==="]
    ...
    with ser:
        ...
        while time.monotonic() < end_time:
            ...
            if waiting:
                ...
                time.sleep(0.02)
            # stop early if nothing new arrived for 2s straight (response is done)
