...
from app.profile_builder import build_profile  # noqa: E402
...
def main() -> int:
    ...
    points = build_profile(session.readings, sample_interval_s=1.0)
    print(f"Built {len(points)} profile points. "
          f"First: {points[0]}  Last: {points[-1]}")

    run = database.create_run(conn, plant="Test Plant", tunnel="Superba A", points=points)
    print(f"\nCreated run: id={run.id} plant={run.plant} tunnel={run.tunnel} "
          f"peak={run.peak_temp_c:.2f}C min={run.min_temp_c:.2f}C "
    ...
    print(f"\nRecent runs: {[(r.id, r.tunnel, r.measurement_count) for r in recent_runs]}")
